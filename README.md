...
- 内核级挂起秒开与工作集冻结：关闭窗口后 WebView2 线程完全挂起，0% CPU 且锁定物理内存，唤醒即刻秒开
- 通过系统托盘菜单快速切换代理组与节点
...
| 关闭管理窗口 | 约 20-40 MB | 内核级挂起（Freeze）WebView2 进程 |
...
- [x] 优化管理窗口的启动速度与常驻内存（已实现 WebView2 原生内核级挂起、即时秒开与物理工作集压缩）
- [ ] 支持 Windows ARM64 架构
