...
- 可选「快速唤醒」：关闭窗口后常驻后台秒开面板，并自动修剪物理工作集内存
- 通过系统托盘菜单快速切换代理组与节点
...
| 状态 | 内存占用 | 行为 |
| --- | ---: | --- |
| 仅运行托盘（冷态） | 不超过 20 MB | 核心和托盘保持运行 |
| 打开管理窗口 | 约 100-300 MB | 使用系统 WebView2 显示前端 |
| 关闭管理窗口（开启快速唤醒） | 约 35-70 MB（增加 20-50 MB） | 窗口常驻后台秒开，自动修剪工作集释放物理 RAM |
| 关闭管理窗口（关闭快速唤醒） | 立即释放窗口内存（不超过 20 MB） | WebView2 被彻底销毁，核心和托盘继续运行 |

> [!TIP]
> 可以在 **「核心」->「行为」** 中自由切换「快速唤醒」开关：
> - **开启快速唤醒（默认）**：托盘唤起仅需 10~30 毫秒（即时秒开），关闭后自动修剪物理工作集，常驻内存仅增加 20-50 MB。
> - **关闭快速唤醒**：追求极致省内存，关闭面板立即销毁整个 WebView2 进程树，托盘内存保持在 20 MB 以内。
...
- [x] 优化管理窗口的启动速度（已实现「快速唤醒」与物理工作集压缩）
- [ ] 支持 Windows ARM64 架构
